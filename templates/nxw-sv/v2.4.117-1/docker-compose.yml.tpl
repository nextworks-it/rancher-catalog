...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.117-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  restifier:
    image: docker-registry.nextworks.it/restifier:v1.2.4
    restart: on-failure
    network_mode: host
    environment:
      OSRD_SERVICE_HOST: osrd
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.6.1
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    environment:
...
# ----------
  ...
  siabot:
    image: docker-registry.nextworks.it/siabot:v0.8.2
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.2.128
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  analytic:
    image: docker-registry.nextworks.it/analytic:v0.6.3
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    #ports:
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.36.4
    #hostname: avo
    environment:
...
# ----------
  ...
  osrd:
    image: docker-registry.nextworks.it/osrd:v1.7.1
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    #ports:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.117-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.60.13
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - OSRD_SERVICE_HOST=osrd
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.0.19"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.12"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
