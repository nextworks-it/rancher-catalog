...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.77-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.4.2
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "8345:8345"
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.2.118.1
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.77-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.60.9
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - OSRD_SERVICE_HOST=osrd
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.0.9"
    restart: on-failure
...
# ------------
  dataporter-edge:
    image: docker-registry.nextworks.it/dataporter-edge:v0.4.12
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
      - EXTERNAL_IFACE=eth0
