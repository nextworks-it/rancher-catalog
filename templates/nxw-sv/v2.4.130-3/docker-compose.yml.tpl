...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.130-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.1.6
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  osrd:
    image: docker-registry.nextworks.it/osrd:v1.7.3
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    #ports:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.130-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.16"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ----------
  dynamo:
    image: docker-registry.nextworks.it/dynamo:v0.4.11-1
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    environment:
...
# ------------
  wurca:
    image: docker-registry.nextworks.it/wurca:v7.43.14
    environment:
