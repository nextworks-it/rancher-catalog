...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.145-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.1.12
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.3.5
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.37.13
    #hostname: avo
    environment:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.145-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.0.24"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.21"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
