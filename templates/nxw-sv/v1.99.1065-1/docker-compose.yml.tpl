...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
    - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:1.99.1065-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.9.114
    restart: on-failure
    network_mode: host
    dns: 169.254.169.250
    environment:
  ...
  tex-server:
    image: docker-registry.nextworks.it/template-expander:v0.0.6
    restart: always
...
# ----------
  ...
  conditions-gateway:
    image: docker-registry.nextworks.it/conditions-gateway:v0.0.14
    restart: on-failure
    ports:
      - '8001:8001'
    environment:
...
# ----------
  ...
  channel-manager:
    image: docker-registry.nextworks.it/channel-manager:v0.9.122
    dns: 169.254.169.250
    environment:
      - CHANNEL_MANAGER_SERVICE_HOST=channel-manager
      - ETCD_SERVICE_HOST=nxw-sv
