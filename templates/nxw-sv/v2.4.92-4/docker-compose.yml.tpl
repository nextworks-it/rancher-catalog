...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.92-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
    ...
    labels:
      ...
      it.nextworks.dependencies: "oamd >= v1.6.8, oamd < v2.0.0-1"
    environment:
...
# ----------
  ...
  rabbitmq-data:
    image: "docker-registry.nextworks.it/rabbitmq-data:rabbitmq-server_v3.11-v0.0.5"
    restart: on-failure
    volumes:
      - rabbitmq-init:/etc/rabbitmq
    labels:
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.1.4
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  restifier:
    image: docker-registry.nextworks.it/restifier:v1.2.2
    restart: on-failure
    network_mode: host
    environment:
      OSRD_SERVICE_HOST: osrd
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.6.0
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    environment:
...
# ----------
  ...
  siabot:
    image: docker-registry.nextworks.it/siabot:v0.7.10.1
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.5.0"
    restart: on-failure
    network_mode: host
...
# NOTE: Keep the following service name as 'mqtt' until SP-880 is fixed
  ...
  mqtt:
    image: docker-registry.nextworks.it/http-mqtt:v1.3.1
    restart: on-failure
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
...
# --------------
  ...
  chrono-manager:
    image: docker-registry.nextworks.it/chrono-manager:v1.1.4
    restart: on-failure
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.2.122
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  favo:
    image: docker-registry.nextworks.it/favo:v0.37.0
    #hostname: favo
    environment:
      ETCD_SERVICE_HOST: nxw-etcd
      ETCD_SERVICE_PORT: '2379'
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.36.0
    #hostname: avo
    environment:
...
# --------
  ...
  av-cfg-fe:
    image: docker-registry.nextworks.it/av-cfg-fe:v1.7.11
    environment:
      OSRD_SERVICE_HOST: osrd
      OSRD_SERVICE_GRPC_PORT: '8345'
      PPROF_ENABLED: 'false'
...
# --------
  ...
  cctv-backend:
    image: docker-registry.nextworks.it/cctv-backend:v0.2.3
    environment:
      ETCD_SERVICE_TTL: '60'
      EXTERNAL_IFACE: eth0
      REDIS_SERVICE_HOST: nxw-sv
...
# ----------
  ...
  osrd:
    image: docker-registry.nextworks.it/osrd:v1.6.2
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    #ports:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.92-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.60.11
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - OSRD_SERVICE_HOST=osrd
...
# ----------
  ...
  license-manager:
    image: "docker-registry.nextworks.it/license-manager-be:v0.6.5"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.0.12"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.2"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ----------
  ...
  posd:
    image: "docker-registry.nextworks.it/posd:v0.7.6"
    restart: on-failure
    network_mode: host
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
...
# ----------
  wake-on-lan:
    image: docker-registry.nextworks.it/wake-on-lan:v0.2.0
    restart: on-failure
    network_mode: host
...
# ----------
  phal:
    image: docker-registry.nextworks.it/phal:v1.3.11
    restart: on-failure
    network_mode: host
    stdin_open: true
    dns: 127.0.0.1
...
# ----------
  dynamo:
    image: docker-registry.nextworks.it/dynamo:v0.4.10-9
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    environment:
...
# ------------
  dataporter-edge:
    image: docker-registry.nextworks.it/dataporter-edge:v0.4.13
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
      - EXTERNAL_IFACE=eth0
...
# ------------
  ringbell:
    image: docker-registry.nextworks.it/ringbell:v4.1.1
    environment:
