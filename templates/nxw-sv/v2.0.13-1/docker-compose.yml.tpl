...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.0.13-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v1.0.18
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.11.4
    restart: on-failure
    network_mode: host
...
# ----------
  ...
  mqtt:
    image: docker-registry.nextworks.it/http-mqtt:v1.0.3
    restart: on-failure
    environment:
...
# ----------
  ...
  conditions-gateway:
    image: docker-registry.nextworks.it/conditions-gateway:v0.0.24
    restart: on-failure
    ports:
      - '8001:8001'
    environment:
...
# ----------
  ...
  channel-manager:
    image: docker-registry.nextworks.it/channel-manager:v0.9.140
    # DNS points to dnrd on nxw-sv.
    # Using the ipsec ip address "attached" to docker0 interface
    network_mode: host
    dns: 127.0.0.1
