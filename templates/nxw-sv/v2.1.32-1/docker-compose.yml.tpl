...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.1.32-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v1.0.34
    restart: on-failure
...
# ----------
  ...
  siabot:
    image: docker-registry.nextworks.it/siabot:v0.7.2
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.17.2
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# NOTE: Keep the following service name as 'mqtt' until SP-880 is fixed
  ...
  mqtt:
    image: docker-registry.nextworks.it/http-mqtt:v1.1.24
    restart: on-failure
    environment:
       - ETCD_SERVICE_HOST=nxw-sv
       - ETCD_SERVICE_PORT=2379
...
# --------------
  ...
  chrono-manager:
    image: docker-registry.nextworks.it/chrono-manager:v0.99.25
    restart: on-failure
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.0.8
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  analytic:
    image: docker-registry.nextworks.it/analytic:v0.1.22
    restart: on-failure
    network_mode: managed
    #ports:
    #  - '8058:8058'
...
# --------
  ...
  favo:
      image: docker-registry.nextworks.it/favo:v0.33.0
      #hostname: favo
      environment:
...
# --------
  ...
  avo:
      image: docker-registry.nextworks.it/avo:v0.31.4
      #hostname: avo
      environment:
...
# --------
  ...
  av-cfg-fe:
      image: docker-registry.nextworks.it/av-cfg-fe:v1.5.1
      environment:
        ETCD_SERVICE_HOST: nxw-sv
        ETCD_SERVICE_PORT: '2379'
        PPROF_ENABLED: 'false'
