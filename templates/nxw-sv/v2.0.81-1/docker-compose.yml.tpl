...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.0.81-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v1.0.31
    restart: on-failure
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.3.2
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "8345:8345"
...
# ----------
  ...
  siabot:
    image: docker-registry.nextworks.it/siabot:v0.6.1
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.4.17"
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.15.12
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# NOTE: Keep the following service name as 'mqtt' until SP-880 is fixed
  ...
  mqtt:
    image: docker-registry.nextworks.it/http-mqtt:v1.1.22
    restart: on-failure
    environment:
       - ETCD_SERVICE_HOST=nxw-sv
       - ETCD_SERVICE_PORT=2379
       - SERVICES=alarm-manager:http://alarm-manager:8086,scenario-manager:http://nxw-sv:8910,supervisor-console:http://nxw-sv:80
...
# ----------
  ...
  conditions-gateway:
    image: docker-registry.nextworks.it/conditions-gateway:v0.0.29
    restart: on-failure
    network_mode: managed
    #ports:
    #  - '8001:8001'
...
#
#  fvstorage:
#    image: docker-registry.nextworks.it/fvstorage:v0.9.11
#    restart: on-failure
#    network_mode: host
#    # dns points to dnrd on nxw-sv, which runs in network nost mode
#    dns: 127.0.0.1
...
#    #  - '6996:6996'
#    environment:
#      - FVSTORAGE_SERVICE_HOST=fvstorage
