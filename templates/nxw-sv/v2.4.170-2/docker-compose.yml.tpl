...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.170-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.2.1
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.5.8"
    restart: on-failure
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.23.1
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  cctv-backend:
    image: docker-registry.nextworks.it/cctv-backend:v0.6.1
    environment:
      ETCD_SERVICE_TTL: '60'
      EXTERNAL_IFACE: eth0
      REDIS_SERVICE_HOST: nxw-sv
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.170-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  license-manager:
    image: "docker-registry.nextworks.it/license-manager-be:v0.6.14"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.2.0"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.4.16"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ------------
  wurca:
    image: docker-registry.nextworks.it/wurca:v7.44.2.1
    environment:
...
# ------------
  nxw-vem:
    image: docker-registry.nextworks.it/nxw.hermes.vem:v3.1.9
    environment:
      OSRD_SERVICE_HOST: osrd
      OSRD_SERVICE_REST_PORT: '8346'
      OPENAI_API_KEY: g
...
# ------------
  nxw-smarttags-be:
    image: docker-registry.nextworks.it/nxw-smarttags-be:v1.2.18
    environment:
      SERVER_HOST: 0.0.0.0
      SERVER_PORT: '8068'
      GRPC_HOST: 0.0.0.0
