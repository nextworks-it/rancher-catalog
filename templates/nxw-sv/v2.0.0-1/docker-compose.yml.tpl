...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.0.0-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v1.0.9
    restart: on-failure
    ports:
      - '7110:7110/tcp'
    environment:
...
# ----------
  ...
  channel-manager:
    image: docker-registry.nextworks.it/channel-manager:v0.9.137
    # DNS points to dnrd on nxw-sv.
    # Using the ipsec ip address "attached" to docker0 interface
    network_mode: host
    dns: 127.0.0.1
