...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.0.59-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v1.0.24
    restart: on-failure
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.2.6
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "8345:8345"
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.4.10"
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.15.10
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# NOTE: Keep the following service name as 'mqtt' until SP-880 is fixed
  ...
  mqtt:
    image: docker-registry.nextworks.it/http-mqtt:v1.1.20
    restart: on-failure
    environment:
       - ETCD_SERVICE_HOST=nxw-sv
       - ETCD_SERVICE_PORT=2379
