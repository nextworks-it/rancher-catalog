...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.123-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  fuseki:
    image: "docker-registry.nextworks.it/fuseki:v4.3.1_2"
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "3030:3030"
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.2.130
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  favo:
    image: docker-registry.nextworks.it/favo:v0.38.0
    #hostname: favo
    environment:
      ETCD_SERVICE_HOST: nxw-etcd
      ETCD_SERVICE_PORT: '2379'
...
# --------
  ...
  av-cfg-fe:
    image: docker-registry.nextworks.it/av-cfg-fe:v1.8.2
    environment:
      OSRD_SERVICE_HOST: osrd
      OSRD_SERVICE_GRPC_PORT: '8345'
      PPROF_ENABLED: 'false'
...
# --------
  ...
  cctv-backend:
    image: docker-registry.nextworks.it/cctv-backend:v0.2.4
    environment:
      ETCD_SERVICE_TTL: '60'
      EXTERNAL_IFACE: eth0
      REDIS_SERVICE_HOST: nxw-sv
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.123-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.14"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ------------
  wurca:
    image: docker-registry.nextworks.it/wurca:v7.42.15
    environment:
