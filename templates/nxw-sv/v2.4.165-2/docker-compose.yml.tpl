...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.165-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.1.18
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  restifier:
    image: docker-registry.nextworks.it/restifier:v1.2.8
    restart: on-failure
    network_mode: host
    environment:
      OSRD_SERVICE_HOST: osrd
    ...
    dns: 127.0.0.1
    volumes:
      - nxw-sv-data:/opt/src/restifier/tmp
    ports:
      - 4995:4995/tcp
    #labels:
    #  io.rancher.container.pull_image: always
...
# ----------
  ...
  fuseki:
    image: "docker-registry.nextworks.it/fuseki:v4.3.1_5"
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "3030:3030"
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.5.5"
    restart: on-failure
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.21.6
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.3.11
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  favo:
    image: docker-registry.nextworks.it/favo:v0.40.0
    #hostname: favo
    environment:
      ETCD_SERVICE_HOST: nxw-etcd
      ETCD_SERVICE_PORT: '2379'
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.37.14
    #hostname: avo
    environment:
...
# --------
  ...
  cctv-backend:
    image: docker-registry.nextworks.it/cctv-backend:v0.6.0
    environment:
      ETCD_SERVICE_TTL: '60'
      EXTERNAL_IFACE: eth0
      REDIS_SERVICE_HOST: nxw-sv
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.165-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.70.3
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - OSRD_SERVICE_HOST=osrd
...
# ----------
  ...
  license-manager:
    image: "docker-registry.nextworks.it/license-manager-be:v0.6.11"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v3.0.32"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.4.2"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
    ...
    environment:
      ...
      - GRPC_DNS_RESOLVER=native
      - ENABLE_SEND_EM_GENERIC_EVENTS=false
      - SUBSCRIBE_SERVER_LOG_LEVEL=info
      - ENABLE_SUBSCRIBE_API=false
    depends_on:
      - fluent-bit
      - spa-config
    ports:
...
# ----------
  ...
  ais:
    ...
    environment:
      ...
      - OSRD_ENABLE_DEBUG=false
      - GRPC_DNS_RESOLVER=native
      - RABBITMQ_SERVICE_HOST=rabbitmq.external
      - RABBITMQ_SERVICE_PORT=5672
      - ENABLE_SEND_EM_GENERIC_EVENTS=false
      - RUN_EM_GENERIC_EVEMT_TESTS=false
      - SUBSCRIBE_SERVER_LOG_LEVEL=info
      - ENABLE_SUBSCRIBE_API=false
    depends_on:
      - fluent-bit
...
# ----------
  ...
  posd:
    image: "docker-registry.nextworks.it/posd:v0.7.13"
    restart: on-failure
    network_mode: host
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
...
# ----------
  phal:
    image: docker-registry.nextworks.it/phal:v1.4.1
    restart: on-failure
    network_mode: host
    stdin_open: true
    dns: 127.0.0.1
...
# ------------
  dataporter-edge:
    image: docker-registry.nextworks.it/dataporter-edge:v0.4.19
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
      - EXTERNAL_IFACE=eth0
...
# ------------
  wurca:
    image: docker-registry.nextworks.it/wurca:v7.44.2
    environment:
...
# ------------
  nxw-vem:
    image: docker-registry.nextworks.it/nxw.hermes.vem:v3.1.7-8
    environment:
      OSRD_SERVICE_HOST: osrd
      OSRD_SERVICE_REST_PORT: '8346'
      OPENAI_API_KEY: g
      EM_URL: http://nxw-sv:8021
      LS_URL: http://nxw-sv:8029
      REST_PORT: '16363'
      GRPC_PORT: '16364'
      ETCD_SERVICE_HOST: nxw-etcd
      ETCD_SERVICE_PORT: '2379'
    ...
    stdin_open: true
    volumes:
    - vem-storage:/code/server/storage
    dns:
    - 127.0.0.1
    tty: true
    logging:
...
# ------------
  nxw-smarttags-be:
    image: docker-registry.nextworks.it/nxw-smarttags-be:v1.2.17
    environment:
      SERVER_HOST: 0.0.0.0
      SERVER_PORT: '8068'
      GRPC_HOST: 0.0.0.0
      ...
      OSRD_SERVICE_GRPC_PORT: '8345'
      LOGGER_LEVEL: TRACE
      SMARTTAGS_SERVICE_REST_PORT: '50059'
      SMARTTAGS_SERVICE_GRPC_PORT: '50058'
      SMARTTAGS_CONFIG_REST_PORT: '8068'
      REST_HOST: 0.0.0.0
      REST_PORT: '50059'
      ETCD_SERVICE_HOST: nxw-etcd
      ETCD_SERVICE_PORT: '2379'
    ...
    network_mode: host
    volumes:
    - data-smarttags:/usr/share/smart_tags
    dns:
    - 127.0.0.1
    logging:
      driver: syslog
