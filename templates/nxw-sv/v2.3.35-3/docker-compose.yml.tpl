...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.3.35-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.0.8
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.20.5
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------------
  ...
  chrono-manager:
    image: docker-registry.nextworks.it/chrono-manager:v1.0.2
    restart: on-failure
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.32.12
    #hostname: avo
    environment:
...
# --------
  ...
  nxw-etcd:
    image: docker-registry.nextworks.it/etcd-nxw:etcd_3_3_27-v0.0.4
    network_mode: managed
    depends_on:
      - nxw-sv
    environment:
      ...
      SUPERVISOR_SIG_SOCK: /hermes/etcd.d/etcd_signal.sock
...
# --------
  ...
  cctv-backend:
    image: docker-registry.nextworks.it/cctv-backend:v0.1.0
    environment:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.3.35-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.51.0
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - EXTERNAL_IFACE=eth0
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v2.3.5"
    restart: on-failure
...
# ----------
  ...
  posd:
    image: "docker-registry.nextworks.it/posd:v0.6.15"
    restart: on-failure
    network_mode: host
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
