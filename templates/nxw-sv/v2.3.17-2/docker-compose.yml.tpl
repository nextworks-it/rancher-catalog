...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.3.17-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.0.3
    restart: on-failure
...
# ----------
  ...
  siabot:
    image: docker-registry.nextworks.it/siabot:v0.7.6
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.4.31-test1"
    restart: on-failure
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.19.1
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# ----------
  ...
  fvstorage:
    image: docker-registry.nextworks.it/fvstorage:v1.2.105
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# --------
  ...
  favo:
      image: docker-registry.nextworks.it/favo:v0.35.2
      #hostname: favo
      environment:
...
# --------
  ...
  avo:
      image: docker-registry.nextworks.it/avo:v0.32.4
      #hostname: avo
      environment:
...
# --------
  ...
  av-cfg-fe:
      image: docker-registry.nextworks.it/av-cfg-fe:v1.6.9
      environment:
        ETCD_SERVICE_HOST: nxw-sv
        ETCD_SERVICE_PORT: '2379'
        PPROF_ENABLED: 'false'
...
# ----------
  ...
  osrd:
    image: docker-registry.nextworks.it/osrd:v0.7.0
    restart: on-failure
    network_mode: managed
    #ports:
    #  - '8001:8001'
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.3.17-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.30.1
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
      - ETCD_SERVICE_PORT=2379
      - EXTERNAL_IFACE=eth0
...
# ----------
  ...
  license-manager:
    image: "docker-registry.nextworks.it/license-manager-be:v0.5.6"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v2.3.3"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v2.2.6"
    restart: on-failure
    network_mode: managed
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
...
# ----------
  ...
  posd:
    image: "docker-registry.nextworks.it/posd:v0.6.9"
    restart: on-failure
    network_mode: host
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
