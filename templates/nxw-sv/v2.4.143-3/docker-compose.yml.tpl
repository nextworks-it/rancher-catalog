...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.4.143-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.37.11
    #hostname: avo
    environment:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.4.143-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v3.1.18"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ------------
  dataporter-edge:
    image: docker-registry.nextworks.it/dataporter-edge:v0.4.17
    environment:
      - ETCD_SERVICE_HOST=nxw-sv
      - EXTERNAL_IFACE=eth0
