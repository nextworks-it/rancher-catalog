...
# ----------
  ...
  nxw-sv:
    ...
    security_opt:
      - seccomp:unconfined
    image: docker-registry.nextworks.it/nxw-sv:2.3.57-1
    restart: on-failure
    stdin_open: true
    network_mode: host
    hostname: supervisor
    ...
    labels:
      ...
      #io.rancher.container.pull_image: always
      it.nextworks.dependencies: "oamd >= v1.6.13, oamd < v2.0.0-1"
    environment:
...
# ----------
  ...
  panels-factory-be:
    image: docker-registry.nextworks.it/panels-factory-be:v2.0.12
    restart: on-failure
    network_mode: managed
    ports:
      - '7110:7110/tcp'
...
# ----------
  ...
  bundle-manager:
    image: docker-registry.nextworks.it/bundle-manager:v0.3.9
    restart: on-failure
    network_mode: managed
    #ports:
    #  - "8345:8345"
...
# ----------
  ...
  plinsky:
    image: "docker-registry.nextworks.it/plinsky:v0.4.31-plin118"
    restart: on-failure
    network_mode: host
...
# ----------
  ...
  alarm-manager:
    image: docker-registry.nextworks.it/alarm-manager:v0.20.7
    restart: on-failure
    network_mode: host
    # dns points to dnrd on nxw-sv, which runs in network nost mode
    dns: 127.0.0.1
...
# ----------
  ...
  channel-manager:
    image: docker-registry.nextworks.it/channel-manager:v0.9.206
    restart: on-failure
    # DNS points to dnrd on nxw-sv.
    # Using the ipsec ip address "attached" to docker0 interface
    network_mode: managed
...
# --------------
  ...
  chrono-manager:
    image: docker-registry.nextworks.it/chrono-manager:v1.0.4
    restart: on-failure
...
# --------
  ...
  favo:
    image: docker-registry.nextworks.it/favo:v0.35.9
    #hostname: favo
    environment:
...
# --------
  ...
  avo:
    image: docker-registry.nextworks.it/avo:v0.32.21
    #hostname: avo
    environment:
...
# ----------
  ...
  osrd:
    image: docker-registry.nextworks.it/osrd:v1.4.1-1
    restart: on-failure
    network_mode: host
    dns: 127.0.0.1
    #ports:
...
# ----------
  ...
  fluent-bit-data:
    image: docker-registry.nextworks.it/fluent-bit-data:v2.3.57-1
    network_mode: "none"
    labels:
      io.rancher.container.hostname_override: container_name
      io.rancher.scheduler.affinity:container_label_soft_ne: io.rancher.stack_service.name=$${stack_name}/$${service_name}
...
# ----------
  ...
  uhal:
    image: docker-registry.nextworks.it/uhal:v0.51.5
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
      - ETCD_SERVICE_PORT=2379
      - EXTERNAL_IFACE=eth0
...
# ----------
  ...
  spa-config:
    image: "docker-registry.nextworks.it/spa-config:v2.3.8"
    restart: on-failure
...
# ----------
  ...
  sem:
    image: "docker-registry.nextworks.it/spa-server:v2.2.17"
    restart: on-failure
    # FIXME: The following network crap is a necessary evil to let the container
    # resolve osrd.external
    network_mode: host # crap: was managed
...
# ----------
  ...
  posd:
    image: "docker-registry.nextworks.it/posd:v0.6.25"
    restart: on-failure
    network_mode: host
    environment:
      - ETCD_SERVICE_HOST=nxw-etcd
